...
| 7b | none | checked, conditional (fallback) | scalar | local | assign | — | — | **rejected today**: `arr[oob] \|\| -1` fails "logical OR in value position requires a conditional left operand" (`conditionPropagates` excludes checked failure) | **S (decided behavior change; semantics doc "Checked-access fallback")** | — | regressions when implemented: `x = arr[oob] \|\| -1` → `-1`, in-bounds zero → `0`; heap-valued (`sarr[oob] \|\| "d"`), ranged, and collector-cell checked fallbacks | 5 | condLHS spine |
| 7c | — | checked, conditional (fallback) | scalar | none | print | — | — | as 7b in print position: `arr[oob] \|\| -1, val1` emits `-1 val1` once per-slot printing exists | **S (decided behavior change; semantics doc)** | — | regression when implemented | 6 | condLHS spine |
