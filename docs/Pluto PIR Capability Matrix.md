...
- **2** — `compileAssignments` → `commitAssignments`; **now planned** at script root for ordinary expressions (string literals including multiline ones, concatenations, inline array literals, binding reads) — block-layout literals stay legacy (rows 2c, 36). *Tests:* `mem/mem.spt` (incl. widened bindings and the empty reset into a differently typed binding), `str`, `multiline_str`, `array_concat`, `cond_copy`; goldens `TestPlanGoldenStaticIntoHeap`, `TestPlanGoldenArrays`, `TestPlanGoldenEffectiveStorage*`, `TestPlanGoldenMultilineString` in `compiler/pir_test.go`. *Helpers:* `commitAssignments`
