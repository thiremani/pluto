...
which step migrates it, and which legacy code its migration finally releases.
The capability router keys on the same columns.
...
The final column lists the **PIR-removable orchestration helpers** a row uses.
Primitives that survive migration — call lowering, loop and guard emission,
storage — are not listed, and a helper can be deleted only once its last row
has migrated. The helper-to-release-step inventory is plan §16 Step 9 (9a
statement-only, 9b orchestration); read the two together rather than treating
any single row as a deletion trigger.

| # | Gate | RHS flags | Value kind | Target | Form | Domain role | Callee effect | Legacy route | Disp | Existing tests | Missing | Step | Removable helpers used |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| 5 | none | ordinary | scalar | blank (`_`) | assign | — | — | ordinary binding, CFG-exempt | **S** (real sink, §4) | `discard` | repeated blanks, mixed types, heap blank, repeated statements, gated/ranged blank | own PR, then 3 | §4 |
...
| 8b | none | call | Range descriptor | local | assign | — (no domain) | all-`MustWrite` | call lowering + indirect-return ABI, not descriptor copying | R | `range_finalize:38` (`makeRange`), `mem/gate_heap` | — | 4 | — |
| 9 | none | ranged | scalar, self-ref | local | assign | RHS-local | — | `compileAssignments` → expression loop nest (passes nil conditions) | R | `math/range_expr`, `math/range.spt`, `range_shadow.spt`, `cond/domain_activation` | — | 7 | `compileAssignments` |
...
| 11 | none | collector, ranged | scalar, heap | local | assign | collector-local | — | `compileArrayExpression` → `compileArray` → `withCollectorDomain` | S | `range`, `array/array_capture`, `math/func_nested_range`, `mem/gate_heap` | — | 8 | collector rewrite |
| 12 | none | call, ranged | scalar, heap, multi-output | local | assign | call-site loop (`LoopInside=false`) | both; any-`MayWrite` also needs Step 6 call-result — cutover stays 7 | `compileDirectCallWithRanges` / `compileIndirectCallWithRanges` | S | `math/func_range`, `math/func_array_range`, `mem/mem_alias_refine.spt` | — | 7 | `compileCondExprValue`, `withCollectorPreparedLoopNest` |
...
| 14b | none/scalar | ordinary, collector, checked | scalar, heap | function output | assign | function-owned | — | body driven by a `Range`/`ArrayRange` **parameter**, whose domain wraps the whole body and may execute zero times — this is what weakens the output to `MayWrite` at the boundary | R | `array/array_func.pt`, `array/array_range.pt`, `array/array_scalar_assign.pt` | — | 7 | — |
...
| 29b | — | call | scalar, heap | none | print | — | both; any-`MayWrite` needs the Step 4 validity variant | non-ranged call argument in print | R | `math/print_func.spt:4` (`Square(3)`) | conditional direct-return argument — the case needing the Step 4 validity variant | 6 | — |
...
| 33 | — | call, ranged | scalar | none | print | call-site loop | both; cutover stays 8 | a ranged **call argument**; print never delegates its own argument iteration to a callee, since the solver forces the synthetic print call's `LoopInside` false | R | `math/print_func` | — | 8 | `withCollectorPreparedLoopNest` |
...
| 35d | none | call | struct value | local | assign | — | both | call lowering | R | — | **uncovered**: struct as a parameter or output | 4 | — |
...
| 36d | none | call | table value | local | assign | — | both | call lowering | R | `array/array_func.*` | — | 4 | — |
