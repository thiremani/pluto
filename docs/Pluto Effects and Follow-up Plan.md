...
| Fold the `Promote` transfer into `Move` (after PR #101) | Ownership-model refactor, behavior-preserving, own PR with fresh race and leak-checked runs. `Owned + Move` consumes the outcome; `Borrowed + Move` takes the owner's old value under the promotion rules. "Takes its owner's old value" must be keyed on `Borrowed && Move`, never `Move` alone — moving an owned outcome does not consume the target's old value, which still needs releasing. Preserve: moved borrow's owner replaced and holding heap; one taker per owner; taken value never also dropped; other replaced values released; elaborator `taken` bookkeeping so duplicate borrows give one move then copies. `[transfer]` prints as `[move]`; the eval's `[borrowed=x]` carries the distinction. Keep the swap, duplicate-source, mixed, widened-binding, and invalid-ownership tests |
