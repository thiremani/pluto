...
type TypeSolver struct {
	...
	// Each function's first read, this pass, of an output nothing has typed yet.
	untypedOutputReads map[*ast.FuncStatement]*token.CompileError
...
func NewTypeSolver(sc *ScriptCompiler) *TypeSolver {
	return &TypeSolver{
		...
		untypedOutputReads: make(map[*ast.FuncStatement]*token.CompileError),
...
// function body a type with an unresolved part, whole or inside a range,
// array or table, can still be settled by a later pass, as for an output read
// before the assignment that types it or a recursive call's result. A type
// that never resolves fails convergence instead.
...
func (ts *TypeSolver) validateBracketLiteralShape(al *ast.ArrayLiteral, numCols int) bool {
	valid := true
	if len(al.Headers) > 0 {
		seen := make(map[string]struct{}, len(al.Headers))
		for _, header := range al.Headers {
			if _, exists := seen[header]; exists {
				ts.Errors = append(ts.Errors, &token.CompileError{
					Token: al.Tok(),
					Msg:   fmt.Sprintf("duplicate table column %q", header),
				})
				valid = false
			}
			seen[header] = struct{}{}
	...
	for i, row := range al.Rows {
		...
		ts.Errors = append(ts.Errors, &token.CompileError{
			Token: al.Tok(),
			Msg:   fmt.Sprintf("bracket literal row %d has %d cells, expected %d", i+1, len(row), numCols),
		})
		valid = false
	}
	return valid
...
func commonArrayElemType(colTypes []Type) (Type, bool) {
	...
	for _, colType := range colTypes {
		switch {
		case elemType.Kind() == UnresolvedKind:
			elemType = colType
		case colType.Kind() == UnresolvedKind:
			continue
		case TypeEqual(elemType, colType):
			continue
		case elemType.Kind() == IntKind && colType.Kind() == FloatKind:
			elemType = colType
		case elemType.Kind() == FloatKind && colType.Kind() == IntKind:
			continue
		default:
			return nil, false
		}
...
func (ts *TypeSolver) TypeDotExpression(expr *ast.DotExpression) []Type {
	...
	case Struct:
		...
		ts.Errors = append(ts.Errors, &token.CompileError{
			Token: expr.Tok(),
			Msg:   fmt.Sprintf("unknown field %q on struct %s", expr.Field, leftType.Name),
		})
		return types
	case Table:
		...
		ts.Errors = append(ts.Errors, &token.CompileError{
			Token: expr.Tok(),
			Msg:   fmt.Sprintf("unknown column %q on %s", expr.Field, leftType.String()),
		})
		return types
	default:
...
func (ts *TypeSolver) typeCell(expr ast.Expression, tok token.Token) (Type, bool) {
	...
	if len(tps) != 1 {
		ts.Errors = append(ts.Errors, &token.CompileError{
			Token: tok,
			Msg:   fmt.Sprintf("bracket literal cell produced %d types; expected 1", len(tps)),
		})
...
func (ts *TypeSolver) mergeColType(cur Type, newT Type, colIdx int, tok token.Token) Type {
	...
	if !allowedCollectionElement(newT) {
		ts.Errors = append(ts.Errors, &token.CompileError{Token: tok, Msg: fmt.Sprintf("unsupported bracket literal element type %s in column %d", newT.String(), colIdx)})
		return cur
	}
...
func (ts *TypeSolver) TypeArrayRangeExpression(ax *ast.ArrayRangeExpression, _ bool) []Type {
	...
	if !ok || ts.awaitingType(arrType) {
		// A target still awaiting its type, or its element type, reports
		// nothing, and range handling visits the index of every error-free
		// access.
		if len(ts.Errors) == errorsBefore {
...
func (ts *TypeSolver) TypeIdentifier(ident *ast.Identifier) (t Type) {
	...
	if !ok {
		// An output holds its seed before the body assigns it, but has no
		// type until an assignment or an earlier pass gives it one.
		if walked, isOutput := ts.walkedOutputOwner(ident.Value); isOutput {
			ts.noteUntypedOutputRead(ident, walked.template)
		} else {
			ts.Errors = append(ts.Errors, &token.CompileError{
				Token: ident.Token,
...
// walkedOutputOwner returns the specialization being walked when name is one
// of its outputs.
func (ts *TypeSolver) walkedOutputOwner(name string) (walkedSpecialization, bool) {
	walked, ok := ts.walkedFuncs[ts.FuncNameMangled]
	return walked, ok && slices.Contains(identNames(walked.template.Outputs), name)
}

// noteUntypedOutputRead keeps each function's first untyped output read of
// the pass. A later pass sees the output typed if any assignment types it; if
// none does, the read explains why the function did not converge.
func (ts *TypeSolver) noteUntypedOutputRead(output *ast.Identifier, template *ast.FuncStatement) {
	if _, noted := ts.untypedOutputReads[template]; noted {
		return
	}
	ts.untypedOutputReads[template] = &token.CompileError{
		Token: output.Token,
		Msg:   fmt.Sprintf("cannot infer type of output %q of %s: it is read before any assignment gives it a type", output.Value, template.Token.Literal),
	}
}
...
func (ts *TypeSolver) typeInfixArrayMask(leftType, rightType Type, op string, tok token.Token) {
	...
	if leftType.Kind() == ArrayKind {
		cmpLeft = leftType.(Array).ElemType
	}
...
// boundary. The compiler routes ranged ||/&& trees to per-iteration staging
...
func (ts *TypeSolver) typeArrayScalarInfix(left, right Type, leftIsArr bool, op string, tok token.Token) Type {
	...
	if leftIsArr {
		arrType = left.(Array)
		leftType = arrType.ElemType
		rightType = right
	} else {
		arrType = right.(Array)
		leftType = left
		rightType = arrType.ElemType
	}
...
func (ts *TypeSolver) InferFuncTypes(ce *ast.CallExpression, bodyArgs []Type, mangled string, template *ast.FuncStatement) *FuncInfo {
	...
	if ts.FuncNameMangled != ts.ScriptCompiler.ScriptMangled {
		...
		// A callee with an argument of unknown type is not the cause of its
		// own unresolved outputs; blame goes to where that type went missing.
		if ts.firstUnresolved == nil && f.Sig.ParamTypesInferred() && !f.OutputTypesInferred() {
...
func (ts *TypeSolver) TypeScriptFunc(mangled string, template *ast.FuncStatement, f *FuncInfo) []Type {
	...
	for {
		...
		if !ts.Converging {
			ts.Errors = append(ts.Errors, ts.nonConvergenceError(template))
			return f.Sig.OutTypes
		}
	}
}

// nonConvergenceError explains a pass that changed nothing yet left types
// unresolved. It blames the first function unresolved on its own, else the
// root, naming that function's read of an output nothing typed if it has one.
func (ts *TypeSolver) nonConvergenceError(root *ast.FuncStatement) *token.CompileError {
	blamed := root
	if ts.firstUnresolved != nil {
		blamed = ts.firstUnresolved
	}
	if read, noted := ts.untypedOutputReads[blamed]; noted {
		return read
	}
...
func (ts *TypeSolver) TypeBlock(template *ast.FuncStatement, f *FuncInfo) {
	...
	readOutputs := ts.ScriptCompiler.Compiler.CodeCompiler.templateBodies[templateKey(template)].readOutputs
	for i, id := range template.Outputs {
