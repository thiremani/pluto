...
func (cfg *CFG) destWriteKinds(s *ast.LetStatement) []EventType {
	...
	for _, v := range s.Value {
		...
		span := c.ExprCache[key(c.CurrFuncMangled, v)].ExprLen
		for j := 0; j < span; j++ {
			if maySkip {
...
func (cfg *CFG) hasRangeExpr(e ast.Expression) bool {
	...
	case *ast.Identifier:
		...
		return len(c.ExprCache[key(c.CurrFuncMangled, t)].Ranges) > 0
	case *ast.StringLiteral:
	...
	case *ast.CallExpression:
		if len(c.ExprCache[key(c.CurrFuncMangled, t)].Ranges) > 0 {
			return true
		}
