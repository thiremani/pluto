...
func (c *Compiler) compileInfixBasic(expr *ast.InfixExpression) (res []*Symbol) {
	...
	for i := 0; i < len(left); i++ {
		...
		// Array ⊕ Scalar (or Scalar ⊕ Array): element-wise broadcast
		if l.Type.Kind() == ArrayKind && (r.Type.Kind() == IntKind || r.Type.Kind() == FloatKind) {
			res = append(res, c.compileArrayScalarInfix(expr.Operator, l, r, true))
			continue
		}
		if (l.Type.Kind() == IntKind || l.Type.Kind() == FloatKind) && r.Type.Kind() == ArrayKind {
...
// compileArrayScalarInfix computes result = array (op) scalar (or scalar (op) array).
// If arrOnLeft is true, compute as elem (op) scalar; else scalar (op) elem.
func (c *Compiler) compileArrayScalarInfix(op string, arrSym *Symbol, scalarSym *Symbol, arrOnLeft bool) *Symbol {
	arrT := arrSym.Type.(Array)
	// Determine result element type via promotion
	arrElem := arrT.ColTypes[0]
	scalarKind := scalarSym.Type.Kind()
	var resElem Type
	if arrElem.Kind() == FloatKind || scalarKind == FloatKind {
		resElem = Float{Width: 64}
	} else {
		resElem = Int{Width: 64}
	}

	// Choose typed runtime functions based on array element
	var lenFn llvm.Value
	var getFn llvm.Value
	var setFn llvm.Value
	var typedPtrType llvm.Type
	if arrElem.Kind() == FloatKind {
		_, lenFn = c.GetCFunc(PT_F64_LEN)
		_, getFn = c.GetCFunc(PT_F64_GET)
		_, setFn = c.GetCFunc(PT_F64_SET)
		typedPtrType = c.namedOpaquePtr("PtArrayF64")
	} else {
		_, lenFn = c.GetCFunc(PT_I64_LEN)
		_, getFn = c.GetCFunc(PT_I64_GET)
		_, setFn = c.GetCFunc(PT_I64_SET)
		typedPtrType = c.namedOpaquePtr("PtArrayI64")
	}

	// Cast generic i8* to concrete runtime pointer
	typedArr := c.builder.CreateBitCast(arrSym.Val, typedPtrType, "arr_typed")
	// Get length
	var lenTy llvm.Type
	if arrElem.Kind() == FloatKind {
		lenTy = c.GetFnType(PT_F64_LEN)
	} else {
		lenTy = c.GetFnType(PT_I64_LEN)
	}
	n := c.builder.CreateCall(lenTy, lenFn, []llvm.Value{typedArr}, "alen")

	// Create result array of promoted type
	var resArr llvm.Value
	if resElem.Kind() == FloatKind {
		resArr = c.createArrayF64(n)
	} else {
		resArr = c.createArrayI64(n)
	}

	zero := llvm.ConstInt(c.Context.Int64Type(), 0, false)
	one := llvm.ConstInt(c.Context.Int64Type(), 1, false)
	// i = 0
	iPtr := c.createEntryBlockAlloca(c.Context.Int64Type(), "i")
	c.createStore(zero, iPtr, Int{Width: 64})

	fn := c.builder.GetInsertBlock().Parent()
	loopHdr := c.Context.AddBasicBlock(fn, "arr_loop.hdr")
	loopBody := c.Context.AddBasicBlock(fn, "arr_loop.body")
	loopDone := c.Context.AddBasicBlock(fn, "arr_loop.done")
	c.builder.CreateBr(loopHdr)

	// Header: i < n ?
	c.builder.SetInsertPointAtEnd(loopHdr)
	iVal := c.createLoad(iPtr, Int{Width: 64}, "i")
	cond := c.builder.CreateICmp(llvm.IntSLT, iVal, n, "i_lt_n")
	c.builder.CreateCondBr(cond, loopBody, loopDone)

	// Body: get elem, compute, set
	c.builder.SetInsertPointAtEnd(loopBody)
	var elem *Symbol
	if arrElem.Kind() == FloatKind {
		call := c.builder.CreateCall(c.GetFnType(PT_F64_GET), getFn, []llvm.Value{typedArr, iVal}, "getf")
		elem = &Symbol{Val: call, Type: Float{Width: 64}}
	} else {
		call := c.builder.CreateCall(c.GetFnType(PT_I64_GET), getFn, []llvm.Value{typedArr, iVal}, "geti")
		elem = &Symbol{Val: call, Type: Int{Width: 64}}
	}

	leftSym := elem
	rightSym := scalarSym
	if !arrOnLeft {
		leftSym, rightSym = scalarSym, elem
	}
	// Ensure scalar is proper primitive LLVM type (already so), but cast for ops if needed handled by defaultOps
	key := opKey{Operator: op, LeftType: leftSym.Type.String(), RightType: rightSym.Type.String()}
	computed := defaultOps[key](c, leftSym, rightSym, true)

	// Cast if needed for store to result element type
	if resElem.Kind() == FloatKind {
		v := computed.Val
		if computed.Type.Kind() == IntKind {
			v = c.builder.CreateSIToFP(v, c.Context.DoubleType(), "i2f")
		}
		c.builder.CreateCall(c.GetFnType(PT_F64_SET), setFn, []llvm.Value{resArr, iVal, v}, "setf")
	} else {
		v := computed.Val
		if computed.Type.Kind() == FloatKind {
			v = c.builder.CreateFPToSI(v, c.Context.Int64Type(), "f2i")
		}
		c.builder.CreateCall(c.GetFnType(PT_I64_SET), setFn, []llvm.Value{resArr, iVal, v}, "seti")
	}

	// i++ and branch
	inc := c.builder.CreateAdd(iVal, one, "i.inc")
	c.createStore(inc, iPtr, Int{Width: 64})
	c.builder.CreateBr(loopHdr)

	// Done block
	c.builder.SetInsertPointAtEnd(loopDone)

	// Return as generic i8* array symbol with updated schema
	resSym := &Symbol{}
	arrRet := arrT
	arrRet.ColTypes = []Type{resElem}
	// Bitcast to i8* for uniform array representation
	resSym.Val = c.builder.CreateBitCast(resArr, llvm.PointerType(c.Context.Int8Type(), 0), "arr_i8p")
	resSym.Type = arrRet
	return resSym
}

// helper to get function type from value (used for building calls where GetFnType name is not available)
func (c *Compiler) GetFnTypeFromValue(fn llvm.Value) llvm.Type {
	return fn.Type().ElementType()
}
