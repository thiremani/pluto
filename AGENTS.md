...
Requirements: Go `1.27` (`go.mod` names the preferred toolchain, which CI and release jobs enforce exactly; macOS build hosts need macOS 13 or later), plus development libraries and tools for the LLVM major in `.llvm-version` (`llvm-config`, `clang`, `clang++`). On macOS, install the versioned Homebrew formula shown in `README.md`.
